...
// GET: Fetch paginated GalleryImage
export const GET = async (req: Request) => {
  try {
    ...
    // Fetch paginated GalleryImage
    const totalItems = await GalleryImage.countDocuments(); // Total number of items
    const totalPages = Math.ceil(totalItems / validLimit); // Calculate total pages
    const galleries = await GalleryImage.find()
    ...
    console.error("Error fetching GalleryImage:", error);

    // Return error response
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch GalleryImage.",
      },
      { status: 500 }
    );
  }
