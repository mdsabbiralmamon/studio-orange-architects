...
const Page = () => {
  ...
  const categories = ["Woodwork", "ReCrafted", "Artwork"];
  ...
  const handleChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    switch (newValue) {
      case 0: // All products
        ...
        break;
      case 1: // Woodwork
        filteredProducts = products.filter((p) => p.category === "Woodwork");
        setCurrentCategory("Woodwork");
        break;
      case 2: // Artwork
        filteredProducts = products.filter((p) => p.category === "Artwork");
        setCurrentCategory("Artwork");
        break;
      case 3: // ReCrafted
        filteredProducts = products.filter((p) => p.category === "ReCrafted");
        setCurrentCategory("ReCrafted");
        break;
    }
        ...
        <Box sx={{ width: "100%" }}>
          <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
            <Tabs
              ...
              scrollButtons={true}
            >
              {["All", "Woodwork", "Artwork", "ReCrafted", "Store"].map((label, index) => (
                <Tab
                  key={label}
                  label={label}
                  {...a11yProps(index)}
